...
import stpio.EdfFile as EdfFile # To be included by PyMCA software (copyright ESRF)
...
def _read_edf(fname):
	...
	try:
		...
		arr = numpy.empty( (f.NumImages, int(d['Dim_2']), int(d['Dim_1'])), dtype=numpy.uint16)
...
def _process(lock, int_from, int_to, offset, abs_offset, files, prefix, projorder, outfile, dsetname, outshape, 
	...
	for i in range(int_from, int_to + 1):    
		...
		# Sort for time (i.e. before or after projections) in case of flat files:
		t  = t1
		
		# "Trick" to define flat "before" and "after", i.e. 48 hours shift of timestamps:
		if (dsetname == 'exchange/data_white'): 
			if ( int(files[i][-8:-4]) != 0 ):
				t  = t + 48*3600	
				idx = ct_right
				ct_right = ct_right - 1			
			else:
...
def main(argv):          
	...
	Example call to convert all the tomo*.tif* projections to a TDF with no cropping and minimum compression:
		...
		python edf2tdf.py 0 -1 "Z:\\rawdata\\c_1\\tomo\\" "Z:\\work\\c1_compr9.tdf" 0 0 0 0 tomo flat 
		dark True True 1 3 "S:\\conversion.txt"
	...
	if os.path.exists(inpath):
		tomo_files = sorted(glob(inpath + tomoprefix + '*.edf'))
	...
	# Read one file in order to understand image size:
	im = _read_edf(	tomo_files[0] )
	...
	if not skipflat:
		...
		# Flat part
		#							
		flat_files = sorted([fn for fn in glob(inpath + flatprefix + '*.edf*') 
								if not "HST" in os.path.basename(fn)])
		num_flats = len(flat_files)		
			
		if ( num_flats > 0):
			
			# Create acquisition group:				
			im = _read_edf(flat_files[0])
			im = im[crop_top:im.shape[0]-crop_bottom,crop_left:im.shape[1]-crop_right]
			...
			if (compressionFlag):
				dset = f.create_dataset('exchange/data_white', flatshape, im.dtype, 
					...
					compression="gzip", compression_opts=compr_opts, 
		...
		# Dark part:
		#	
		dark_files = sorted([fn for fn in glob(inpath + darkprefix + '*.edf*') 
								if not "HST" in os.path.basename(fn)])		
		num_darks = len(dark_files)				
			
		if ( num_darks > 0):
			im = _read_edf(dark_files[0])			
			im = im[crop_top:im.shape[0]-crop_bottom,crop_left:im.shape[1]-crop_right]
